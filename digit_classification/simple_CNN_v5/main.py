...
NUM_CLASSES = 10
KERNEL_SIZE = 3
MODEL_NAME = 'simple_CNN_v5'

class SimpleCNN_v5(nn.Module):
    def __init__(self, num_classes=10):
        super(SimpleCNN_v5, self).__init__()
...
class RemoveLines_v2:
    ...
    def __call__(self, img):
        ...
        inpainted = cv2.inpaint(color, mask, self.inpaint_rad, cv2.INPAINT_NS)
...
def main():
    ...
    train_data, train_labels, val_data, val_labels = load_from_csv(data_folder, data_version=4)
    ...
    model = SimpleCNN_v5(num_classes=NUM_CLASSES).to(device)
